# ==========================================================================================
# Packer Build: BUDGIE AMI on Ubuntu 24.04 (Noble)
# ------------------------------------------------------------------------------------------
# Purpose:
#   - Uses Packer to build a custom Amazon Machine Image (AMI) for BUDGIE XRDP
#   - Starts from the official Canonical Ubuntu 24.04 AMI
#   - Installs prerequisites (SSM agent, AWS CLI, packages, BUDGIE Server)
...
source "amazon-ebs" "budgie_ami" {
  ...
  ami_name      = "budgie_ami_${replace(timestamp(), ":", "-")}" # Timestamped AMI name
  ...
  tags = {
    Name = "budgie_ami_${replace(timestamp(), ":", "-")}" # Tag AMI with unique name
  }
}
...
# ------------------------------------------------------------------------------------------
build {
  sources = ["source.amazon-ebs.budgie_ami"]
